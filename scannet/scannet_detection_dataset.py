...
class ScannetDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', num_points=20000,
                 use_color=False, use_height=False, augment=False, useMaskrcnn=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.useMaskrcnn:
            feature_path = '/home/extra/extra/outputs'
            mesh_vertices = np.load(os.path.join(feature_path, scan_name) + '_features.npy')
            N = mesh_vertices.shape[0]
            if N > 50000:
                choices = np.load(os.path.join(self.data_path, scan_name) + '_choices.npy')
        ...
        if self.use_height:
            ...
            point_cloud = np.insert(point_cloud, [3], height, axis=1)
